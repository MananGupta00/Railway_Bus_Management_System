...
public class Sadd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try { 
            ...
            else if(id.equals("showStation")){
				 //retrieve data from database and respond to

				 PreparedStatement st = con.prepareStatement("select * from stops where stop like '%station%' or stop like '%Station%' or stop like '%STATION%';");
    	    ...
    	    out.print("<div id='addStop' class='section'> <p class='heading'>Add New Stop</p> Stop Name:<input id='stopid' type='text'/> <br /><br/> <button id='stopbut' type='submit'>ADD Stop</button> </div> <hr size='2px'> <div id='add' class='section'> <p class='heading'>ADD BUSES</p> <div id='info'> <div>BUS NUMBER: <input id='busNoid' name='busNo' type='text'/> </div><br /> <div>BUS CONDUCTOR:<input id='conid' name='con' type='text' /> </div><br /> <div>STATION:		<select id='statid' ></select></div><br /> <button id='submitbus'type='submit'>ADD BUS</button> </div> </div> <hr size='2px'>  <script> $('#stopbut').click( function() {  $.get('Sadd',{ 	'id': 'stop', 	'stop': document.getElementById('stopid').value} ).done(function(response) {  alert(response); }); }); $('#submitbus').click( function() {  $.get('Sadd', 		{ 	'id': 'bus', 	'busNo': document.getElementById('busNoid').value, 	'con': document.getElementById('conid').value, 'stat': document.getElementById('statid').value} ).done(function(response) {  alert(response); }); }); $(document).ready(function() {  $.get('Sadd', 		{ 		'id': 'showStation' 			}).done(function(response) {  	$('#statid').html(response); }); }); </script>");
